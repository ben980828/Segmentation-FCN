...
class Sat_Image(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = Image.open(self.fileroot+image_fn).convert('RGB')    
        mask_fn = self.mask_root[index]
        mask = Image.open(self.fileroot+mask_fn).convert('RGB')
...
def main():
    ...
    for ep in range(0, epoch):
        ...
        acc /= valid_data_num
        # if acc > 0.675:
        #     print('Saving epoch for acc : ({:.3f}). Save model '.format(acc))
        #     torch.save(model.state_dict(), 'fcn32s_ep{}_acc{}.pth'.format(ep, acc))

        print('\nTest set: Average loss: {:.4f}'.format(val_loss))
        print('Accuracy = {}\n'.format(acc))
        if (acc > max_iou) and (acc > 0.675):
            print('Performance improved : ({:.3f} --> {:.3f}). Save model ==> '.format(max_iou, acc))
            max_iou = acc
            torch.save(model.state_dict(), 'fcn32s_optimal_acc{}.pth'.format(acc))
        lr_decay.step(acc)
